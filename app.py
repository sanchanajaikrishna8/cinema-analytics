...
def load_or_generate_dataset(samples=1200):
    ...
    actors_pool = ["Keanu Reeves", "Zendaya", "Oscar Isaac", "Cate Blanchett", "Joaquin Phoenix", "Ryan Reynolds", "Emma Stone", "Hiroyuki Sanada"]
    languages_pool = ["English", "Spanish", "French", "Japanese", "German", "Italian", "Portuguese"]
    theaters_pool = ["AMC Lincoln Square", "Regal LA Live", "BFI IMAX London", "Cineworld Leicester Square", "TOHO Cinemas Shinjuku", "AMC Empire 25"]
    ...
    for idx in range(samples):
        ...
        cast_selection = ", ".join(np.random.choice(actors_pool, size=2, replace=False))
        primary_lang = np.random.choice(languages_pool, p=[0.6, 0.1, 0.1, 0.05, 0.05, 0.05, 0.05])
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader("Upload custom CSV dataset", type=["csv"], help="Overwrite backend DB with new movie catalog.")
    if uploaded_file is not None:
        try:
            # Read uploaded CSV
            new_df = pd.read_csv(uploaded_file)
            # Validate basic schema
            required_cols = {'Title', 'Genres', 'Runtime_Min', 'Season', 'Content_Type', 'IMDb_Score', 'TMDB_Popularity'}
...
if df.empty:
    st.warning("The database is currently empty. Please upload a CSV dataset in the sidebar to populate the catalog and train models.")
    st.stop()
...
with tab2:
    ...
    with col_search1:
        search_query = st.text_input("Search movie title or cast", placeholder="Type a movie title or actor (e.g. Horizon, Keanu)...")
    with col_search2:
        genre_filter = st.multiselect("Filter by Genre", options=list(genre_list))
    with col_search3:
        lang_filter = st.multiselect("Filter by Language", options=list(df['Primary_Language'].unique()) if 'Primary_Language' in df.columns else [])

    filtered_df = df.copy()
    if search_query:
        title_match = filtered_df['Title'].str.contains(search_query, case=False, na=False)
        cast_match = filtered_df['Cast'].str.contains(search_query, case=False, na=False) if 'Cast' in filtered_df.columns else False
    ...
    if len(filtered_df) > 0:
        ...
        with card_col2:
            ...
            display_cols = [c for c in ['Title', 'Genres', 'Cast', 'Primary_Language', 'Theaters_Available', 'Runtime_Min', 'Season', 'Content_Type', 'IMDb_Score', 'TMDB_Popularity'] if c in filtered_df.columns]
...
with tab5:
    ...
    with col_a:
        ...
        fig_m.update_traces(line_color='#8C5A3C', line_width=3, marker_size=8)
        fig_m.update_layout(
            plot_bgcolor='rgba(0,0,0,0)',
            paper_bgcolor='rgba(0,0,0,0)',
            font=dict(color='#1E150C', size=13),
            xaxis=dict(title="Release Month", showgrid=True, gridcolor='#EAE0D5'),
            yaxis=dict(title="Average TMDB Popularity", showgrid=True, gridcolor='#EAE0D5'),
            height=400
        )
        st.plotly_chart(fig_m, use_container_width=True)

    with col_b:
        st.subheader("Seasonal Engagement Distribution")
        ...
        fig_s = px.bar(
            ...
            y='TMDB_Popularity',
            color='Season',
            color_discrete_sequence=['#8C5A3C', '#A87654', '#C4936F', '#E2D5C3']
        )
        fig_s.update_layout(
            plot_bgcolor='rgba(0,0,0,0)',
            paper_bgcolor='rgba(0,0,0,0)',
            font=dict(color='#1E150C', size=13),
            xaxis=dict(title="Season", showgrid=True, gridcolor='#EAE0D5'),
            yaxis=dict(title="Average Popularity", showgrid=True, gridcolor='#EAE0D5'),
            showlegend=False,
            height=400
        )
...
with tab6:
    ...
    st.markdown("Configure target parameters below to estimate performance metrics:")
    
    col_in1, col_in2 = st.columns(2)
    
    with col_in1:
        input_genres = st.multiselect("Select Targeted Genres", options=list(genre_list), default=[genre_list[0]])
    ...
    if st.button("🚀 Execute Prediction Model", use_container_width=True):
        if not input_genres:
            st.warning("Please select at least one genre to run the prediction model.")
        else:
            ...
            type_col = f"Content_Type_{input_type}"
            ...
            with res_col1:
                st.metric(
                    ...
                    delta=f"{round(pred_rating - df['IMDb_Score'].mean(), 2)} vs Catalog Avg"
                )
            with res_col2:
                st.metric(
                    label="Predicted TMDB Popularity", 
                    value=f"{pred_pop:.2f}",
                    delta=f"{round(pred_pop - df['TMDB_Popularity'].mean(), 2)} vs Catalog Avg"
                )
...
with tab7:
    ...
    st.markdown("Historical record of all executed model runs stored in SQLite database:")
    ...
    if not history_df.empty:
        ...
        st.info("No predictions recorded yet. Run a prediction in the ML Predictor Engine tab to generate history.")
